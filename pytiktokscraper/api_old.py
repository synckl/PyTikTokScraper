import json
import os.path
from pytiktokscraper.helper import helper
...
class api():
    ...
    def login(self,username,password):
        ...
        if os.path.exists(os.path.join(os.getcwd(), 'cookies', username + ".json")):
    ...
    def user_info(self,user_id = '6594722549190574086', session = {}):
        ...
        if(session.__len__()>0):
            ...
            data = self.helper.request_get(self,url)
        return data.json()
    def user_video_list(self,user_id = '6594722549190574086', max_cursor = 0):
        url = self.api_url + "aweme/v1/aweme/post/?user_id="+str(user_id)+"&type=0&count=20&pull_type=1&max_cursor=" + str(max_cursor) + "&"+self.helper.query(self.helper.default_veriable(self.global_veriable))
        if(self.active_user.get('cookies', None)):
            data = self.helper.request_get(self,url,session=self.active_user['cookies'])
        else:
            data = self.helper.request_get(self,url)
        return data.json()
